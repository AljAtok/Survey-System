    ...
    public function submit(){
		...
		if(!$duplicate_entry && !$conflict_emp){
			...
			if (
				...
				$upload_path = rtrim(str_replace('\\', '/', FCPATH), '/') . '/assets/uploads/or_number/';
				if (!is_dir($upload_path)) {
					mkdir($upload_path, 0755, true);
				}
				@chmod($upload_path, 0777); // Ensure write permission for Linux
				...
				if (preg_match('/^data:image\/([a-zA-Z0-9]+);base64,/', $or_photo, $matches)) {
					...
					if ($image !== false) {
						...
						imagedestroy($image);
						@chmod($target_file, 0666); // Ensure file is writable
					} else {
						$response = [
							'errors' => ['or_photo' => 'Failed to process image data.'],
							'csrf_hash' => $this->refresh_hash()
						];
						$this->output
							->set_status_header(400)
			...
			if(in_array($form_data->form_id, [3, 4, 5])){
				...
				$survey_ref_result = $this->main->insert_data("{$sibling_db}.survey_reference_tbl", $set, TRUE);

				// $debug_log_dir = FCPATH . 'assets/uploads/debug_logs/';
				// if (!is_dir($debug_log_dir)) {
				// 	mkdir($debug_log_dir, 0755, true);
				// }
				// @chmod($debug_log_dir, 0777);
				...
				// 	'name' => $transaction_survey_reference[shorten_field_name('name', null)] ?? '',
				// 	'email' => $transaction_survey_reference[shorten_field_name('email', null)] ?? '',
				// 	'contact_number' => $transaction_survey_reference[shorten_field_name('contact number', null)] ?? '',
				// 	'survey_data' => $survey_data,
				// 	'files' => $_FILES,
				// 	'survey_ref_result' => $survey_ref_result,
				// 	'db_error' => $this->db->error(),
				// ];

				// $debug_filename = $debug_log_dir . 'debug_' . date('Ymd_His') . '_' . uniqid() . '.json';
				// file_put_contents($debug_filename, json_encode($debug_data, JSON_PRETTY_PRINT));
				// @chmod($debug_filename, 0666);
			}
