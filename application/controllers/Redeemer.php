...
class Redeemer extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST' && !$this->input->is_ajax_request()) {
            $employee_id = ($this->session->has_userdata(system_default()['session_name'])) 
                ? clean_data(decode($this->session->userdata(system_default()['session_name'])['employee_id'])) : NULL;
    ...
    public function index()
    {
        ...
        redirect($this->controller . '/transaction_list'); 
        return;
    }
   ...
   public function account_settings()
   {
       ...
       $data['emp_id']     = $info['employee_id'];
       $data['employee']   = $this->main->get_data('employees_tbl', ['employee_id' => $emp_id], TRUE);
       $data['content']    = $this->load->view($this->controller.'/account_setting_content', $data, TRUE);
       $this->load->view($this->controller.'/template', $data);
   }

   public function check_curr_password($field_value)
   {
       $info   = require_login($this->controller);
       $emp_id = decode($info['employee_id']);
       ...
       return password_verify($field_value, $get_emp->employee_password);
   }
   ...
   public function store_new_password()
   {
       ...
       $rules = [
           [ 'field' => 'id'                , 'label' => 'employee_id'         , 'rules' => 'required|callback_check_new_pass_employee' ],
           [ 'field' => 'currentpassword'   , 'label' => 'Current Password'    , 'rules' => 'required|callback_check_curr_password'     ],
           [ 'field' => 'newpassword'       , 'label' => 'New Password'        , 'rules' => 'required'                                  ],
       ...
       foreach ($employees_data as $key => $values) {
           ...
           $action = '
               <div class="d-flex gap-1 align-items-center">
                   <button class="btn btn-primary btn-sm employee-btn" data-component="Edit_Employee_Details" data-id="' . $encoded_id . '" data-url="/update_employee">Edit</button><br>
                   <button class="btn btn-primary btn-sm employee-btn" data-component="Reset_Employee_Password" data-id="' . $encoded_id . '" data-url="/reset_employee">Reset&nbsp;PW</button><br>
   ...
   public function forms() //Displays form page
   {
      require_login($this->controller);
       ...
       $forms_page_resources = [
           'title' => ucwords(str_replace('_', ' ', __FUNCTION__)),
           'forms' => $this->main->get_data('form_tbl', FALSE, FALSE, ['form_id', 'form_name', 'status']),
           'controller' => $this->controller,
       ];
   ...
   {   
       require_login($this->controller);
       $data['forms']            = $this->main->get_data('form_tbl', ['status' => 1], TRUE);

       $transaction_page_resources = [
           'title'         => ucwords(str_replace('_', ' ', __FUNCTION__)),
           'controller'    => $this->controller,
           'forms'         => $data['forms'],
           'js_scripts'    => ['transaction/transaction_list_page.js'],
       ];
   ...
   public function load_transaction_page_table($id){
       ...
       if (!empty($search)) {
           ...
           $filtered_data = array_filter($all_data, function($row) use ($search) {
               return stripos($row->reference_number, $search) !== false ||
       ...
       foreach($paginated_data as $key => $value){
           ...
           if (decode($employee_info['employee_id']) == 115) {//This needs to be changed some time in the future
   ...
   public function survey_export($form_id){
       ...
       $date_from = isset($date_range[0]) ? $date_range[0] : '';
       $date_to = isset($date_range[1]) ? $date_range[1] : '';


       $survey_tag = $form_id == 1 ? 'CDI' : 'CTG';

       $date = formatDateRange($date_from, $date_to);
